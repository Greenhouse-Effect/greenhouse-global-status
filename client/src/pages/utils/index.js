...
export const handleAxios = async (
  ...
  switch (queryType) {
    case 'Select One': {
      if (entity === 'country') {
        return await axios
          .get(`http://35.92.119.149:8080/${entity}/name/${countryName}`)
          .catch((err) => {
            console.log(err);
          });
      } else {
        // return await axios
        //   .get(
        //     `http://35.92.119.149:8080/${entity}/name/${countryName}/year/${year}`
        //   )
        //   .catch((err) => {
        //     console.log(err);
        //   });
    ...
    case 'Compare': {
      if (entity === 'country') {
        return await axios
          .get(
            `http://35.92.119.149:8080/${entity}/${attribute}/${operator}/${slider}`
          )
          .catch((err) => {
            console.log(err);
          });
        ...
        console.log('attribute here ' + attribute);
        console.log('operator here ' + operator);
        console.log('slider here ' + slider);

        return await axios
          .get(
            `http://35.92.119.149:8080/${entity}/year/${year}/${attribute}/${operator}/${slider}`
          )
          .catch((err) => {
            console.log(err);
          });
